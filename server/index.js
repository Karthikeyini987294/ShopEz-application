...
const calculateDeliveryDate = (orderDate) => {
    ...
    delivery.setDate(delivery.getDate() + 5); // You can change 5 to 7 or any value
    return delivery;
}
    ...
    try {
        ...
        for (const item of cartItems) {
            const newOrder = new Orders({
                ...
                mainImg: item.mainImg, size: item.size, quantity: item.quantity,
                price: item.price, discount: item.discount,
                paymentMethod, orderDate, deliveryDate
            });
            await newOrder.save();
